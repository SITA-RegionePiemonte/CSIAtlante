...
from qgis.core import QgsLayerTreeLayer, QgsLayerTreeGroup, QgsLayerTreeNode
...
from . import resources_rc  # @UnusedImport
...
if REMOTE_DBG:
    try:
        ...
        #2to3 PYSRC_PATH = s.value(''.join([PROJECT_NAME, "/pysrc"]), "", type=unicode)
        PYSRC_PATH = s.value(''.join([PROJECT_NAME, "/pysrc"]), "", type=str)
        if PYSRC_PATH != "":
            import sys
...
#     rows = '\n'.join(lines)
    ...
    response = urllib.request.urlopen(url, byte_encoded_args)
...
#         wlog.close()
    ...
    #2to3 if cache_rows != rows:
    #2to3    cache_rows = rows
    #2to3 cache_json = json.loads(cache_rows)

    cache_json = dict_data
...
class CsiAtlantePlugin(QObject):
    ...
    def __init__(self, iface):
        ...
        # Get QGIS version
        #try:
        #2to3    self.QgisVersion = unicode(QGis.QGIS_VERSION_INT)
        #except:
        #2to3    self.QgisVersion = unicode(QGis.QGIS_VERSION)[0]
        try:
            self.QgisVersion = str(Qgis.QGIS_VERSION_INT)
        except:
            self.QgisVersion = str(Qgis.QGIS_VERSION)[0]
        ...
        #2to3 locale = QSettings().value("locale/userLocale")
        #2to3 myLocale = locale[0:2]
        myLocale = 'it'
        ...
        # eventualmente viene sovrascritto alla creazione del dialog
        self.behaviour = 0
    ...
    def initGui(self):
        ...
        #2to3 QObject.connect(self.action_run, SIGNAL("triggered()"), self.slot_run)
        #2to3 QObject.connect(self.action_about, SIGNAL("triggered()"), self.slot_about)
        #2to3 QObject.connect(self.action_chart_identify, SIGNAL("triggered()"), self.slot_chart_identify)
        #2to3 QObject.connect(self.action_graph_identify, SIGNAL("triggered()"), self.slot_graph_identify)
        self.action_run.triggered.connect(self.slot_run)
        self.action_about.triggered.connect(self.slot_about)
        self.action_chart_identify.triggered.connect(self.slot_chart_identify)
        self.action_graph_identify.triggered.connect(self.slot_graph_identify)
        ...
        #2to3 QObject.connect(self.dlg, SIGNAL("visibilityChanged ( bool )"), self.slot_widget_visible)
        #2to3 QObject.connect(self.dlg.ui.btCarica, SIGNAL("clicked()"), self.slot_carica_selezionati)
        #2to3 QObject.connect(self.dlg.ui.btAggiorna, SIGNAL("clicked()"), self.slot_aggiorna_elenco)
        #2to3 QObject.connect(self.dlg.ui.btSvuotaToc, SIGNAL("clicked()"), self.slot_svuota_toc)
        self.dlg.visibilityChanged .connect(self.slot_widget_visible)  # old style Qt signals and slot
        #self.dlg.visibilityChanged[bool].connect(self.slot_widget_visible)                         # new style Qt signals and slot
        self.dlg.ui.btCarica.clicked.connect(self.slot_carica_selezionati)
        self.dlg.ui.btAggiorna.clicked.connect(self.slot_aggiorna_elenco)
        self.dlg.ui.btSvuotaToc.clicked.connect(self.slot_svuota_toc)
...
#       #QObject.connect(self.dlg.ui.indica_btSerieStorica, SIGNAL("clicked()"), self.canvas.setMapTool(self.toolChartSerieStorica))
    ...
    def __disconnect(self):
        ...
        """
        #2to3 QObject.disconnect(self.action_run, SIGNAL("triggered()"), self.slot_run)
        #2to3 QObject.disconnect(self.action_about, SIGNAL("triggered()"), self.slot_about)
        #2to3 QObject.disconnect(self.action_chart_identify, SIGNAL("triggered()"), self.slot_chart_identify)
        #2to3 QObject.disconnect(self.action_graph_identify, SIGNAL("triggered()"), self.slot_graph_identify)
        #2to3 QObject.disconnect(self.dlg, SIGNAL("visibilityChanged ( bool )"), self.slot_widget_visible)
        #2to3 QObject.disconnect(self.dlg.ui.btCarica, SIGNAL("clicked()"), self.slot_carica_selezionati)
        #2to3 QObject.disconnect(self.dlg.ui.btAggiorna, SIGNAL("clicked()"), self.slot_aggiorna_elenco)
        #2to3 QObject.disconnect(self.dlg.ui.btSvuotaToc, SIGNAL("clicked()"), self.slot_svuota_toc)

        self.action_run.triggered.disconnect(self.slot_run)
        self.action_about.triggered.disconnect(self.slot_about)
        self.action_chart_identify.triggered.disconnect(self.slot_chart_identify)
        self.action_graph_identify.triggered.disconnect(self.slot_graph_identify)
        self.dlg.visibilityChanged[bool].disconnect(self.slot_widget_visible)
        self.dlg.ui.btCarica.clicked.disconnect(self.slot_carica_selezionati)
        self.dlg.ui.btAggiorna.clicked.disconnect(self.slot_aggiorna_elenco)
        self.dlg.ui.btSvuotaToc.clicked.disconnect(self.slot_svuota_toc)
        ...
        try:
            ...
            if self.dlg.ui.tabWidget.currentWidget() == self.dlg.ui.tabProgetti:
                ...
                if len(datiCat) > 0:
                    ...
                    self.iface.messageBar().pushMessage("Attenzione", msg, level=Qgis.Critical)
                    csiutils.Cursor.Restore()
...
#                             #self.canvas.setMapTool(self.toolChartInfo)
    ...
    def addWMSLayer(self, wmsselection, crs, newLayersAsGroup, outputformat, abilita, usadesc, stampabile):
        ...
        #2to3 legend = self.iface.legendInterface()
        ...
        s = QSettings()
        featurecountlimit = s.value("/CSIAtlante/wms/featureCount", "10", type=str) #2to3 type=unicode)

        # valori di default per tile size, usati in caso di wms stampabile
        tileMaxHeight = s.value("/CSIAtlante/wms/maxHeight", "256", type=str) #2to3 type=unicode)
        tileMaxWidth = s.value("/CSIAtlante/wms/maxWidth", "256", type=str) #2to3 type=unicode)

        #2to3 legend.blockSignals(True)
        ...
        try:
            if newLayersAsGroup:
                ...
                #2to3 TODO:
                #2to3 legend.setGroupVisible(groupindex, abilita)
                ...
                for i in range(len(layers)):
                    ...
                    if layers[i] != u'<has_no_Name>':
                        ...
                        #2to3 urlWithParams = urllib.unquote_plus(urllib.urlencode(utf8params))
                        urlWithParams = urllib.parse.unquote_plus(urllib.parse.urlencode(utf8params))
                        rlayer = self.iface.addRasterLayer(urlWithParams, layerDesc[i], "wms")
                        if not abilita:
                            #2to3 TODO:
                            #2to3 legend.setLayerVisible(rlayer, False)
                            pass
                        #2to3 TODO:
                        #2to3 legend.moveLayer(rlayer, groupindex)
            else:
                # --------------------------------------------
                # CASO B:
                ...
                if (wmsselection.layers[0].parentLayer is None):
                    ...
                    if layers[i] != u'<has_no_Name>':
                        ...
                        #2to3 TODO:
                        #2to3 groupindex = legend.addGroup(wmsselection.tree.name)
                        #2to3 legend.setGroupVisible(groupindex, abilita)
            ...
            #2to3 TODO:
            #2to3 legend.blockSignals(False)
            csiutils.Cursor.Restore()


#2to3 class AsynchronousHTTPHandler(urllib2.HTTPHandler):
#    """
    ...
    def loadVectorLayerBase(self, iface, readQmlUrl, visible=False, loadQML=False, postfixLegendName=''):
        ...
        if pwdCoax == None or pwdCoax == "":
            ...
            try:
                ...
                s = QSettings()
                oldValidation = s.value("/Projections/defaultBehaviour", "useGlobal", type=str)
                ...
                if vl.isValid():
                    ...
                    # qgis3: QgsMapLayerRegistry. Its functionality has been moved to QgsProject.
                    #2to3 QgsMapLayerRegistry.instance().addMapLayer(vl)
                    QgsProject.instance().addMapLayer(vl, addToLegend=True)

                    # qgis3: 
                    # QgsLayerTreeNode::setItemVisibilityChecked    (    bool     checked    )
                    # QgsLayerTreeNode::setItemVisibilityCheckedParentRecursive    (    bool     checked    )    
                    # QgsLayerTreeNode::setItemVisibilityCheckedRecursive    (    bool     checked    )    
                    #2to3 legend.setLayerVisible(vl, visible)
                ...
                #2to3 legend.blockSignals(False)
                #2to3 legend.refreshLayerSymbology(vl)
                return vl
...
#                QgsMessageLog.logMessage('layer.loadNamedStyle: {}'.format(userFilePath), LOGGER_TAG, Qgis.Info)
    ...
    def loadTree(cl, infolist):
        ...
        s = QSettings()
        category_aux = s.value("CSIAtlante/conf/categoriaAll", "*", type=str) #2to3 u"*", type=unicode)
...
#                 w.ideal_point = 0 if ln.get('ideal_point', 0) == '' else int(ln.get('ideal_point', 0))
    ...
    def loadTree(cl, infolist):
        ...
        # introduce una nuova categoria "*" "All Inclusive" unione di tutte le categorie
        s = QSettings()
        category_aux = s.value("CSIAtlante/conf/categoriaAll", "*", type=str) #u"*", type=unicode)
...
#                 legend.removeGroup(groupindex)
        ...
        if (vl is not None):
            ...
            #2to3 TODO:
            #2to3 legend.setLayerVisible(vl, visible)
            #2to3 legend.moveLayer(vl, groupindex)
            pass
        else:
    ...
    def loadVectorLayerSerieStoricaBase(self, iface, readQmlUrl, visible=False, loadQML=False, postfixLegendName='', tocLayerName='', propertySerieStorica={}):
        ...
        if pwdCoax == None:
            ...
            try:
                # made sure the crs prompt is disabled
                s = QSettings()
                oldValidation = s.value("/Projections/defaultBehaviour", "useGlobal", type=str) #2to3 type=unicode)
                s.setValue("/Projections/defaultBehaviour", "useGlobal")
                """
                La connessione al DB deve avvenire con le credenziali di utente
                Il caricamento del vector layer avviene con le credenziali dello schema
    ...
    def loadRasterLayer(self, iface, readQmlUrl, visible=False, loadQML=False):
        ...
        try:
            ...
            if rlayer.isValid():
                ...
                #2to3 TODO:
                #2to3 legend.setLayerVisible(rlayer, visible)
            ...
            #2to3 TODO:
            #2to3 legend.blockSignals(False)
            #2to3 legend.refreshLayerSymbology(rlayer)
            pass
...
class WMSInfo:
    ...
    def readFromCache(cls, cachefile):
        ...
        try:
            ...
            #2to3 f = open(cachefile, 'r')  # , encoding='utf-8')
            #2to3 wmsInfoList = pickle.load(f, encoding='utf-8')
            #2to3 f.close()

            with open(cachefile, 'rb') as f:                #'rb') as f:
                #2to3 wmsInfoList = pickle.load(f, encoding='utf-8')
                #wmsInfoList = pickle.load(f, fix_imports=True, encoding='utf-8', errors="strict")
                wmsInfoList = pickle.load(f, fix_imports=True, encoding='utf-8')
    ...
    def writeToCache(cls, cachefile, wmsInfoList):
        ...
        try:
            prima = datetime.datetime.now()  # @UnusedVariable
            #2to3 f = open(cachefile, 'w')  # , encoding='utf-8')
            f = open(cachefile, 'wb')  # , encoding='utf-8')
...
class ConnectionsPortfolio(object):
    ...
    def getConnections(self):
        ...
        try:
            ...
            for name in names:
                ...
                #2to3 c.username = s.value(key + "/username", u'', type=unicode)
                #2to3 c.host = s.value(key + "/host", u'', type=unicode)
                #2to3 c.port = s.value(key + "/port", u'', type=unicode)
                #2to3 c.database = s.value(key + "/database", u'', type=unicode)
                #2to3 c.password = s.value(key + "/password", u'', type=unicode)
                c.username = s.value(key + "/username", "", type=str)
                c.host = s.value(key + "/host", "", type=str)
    ...
    def patchConnections():
        ...
        try:
            ...
            # selected
            storedSelected = s.value('/CSIAtlante/connections/selected', '', type=str) #2to3 u'', type=unicode)
    ...
    def compress(cls, tocompress, cover):
        ...
        @return: compressed
        """
        #2to3 covered = ''.join([tocompress.encode('utf-8'), cover.encode('utf-8')])
        covered = b''.join([tocompress.encode('utf-8'), cover.encode('utf-8')])
...
class ConnectionDTO():
    ...
    def __init__(self):
        #2to3 self.name = u''
        #2to3 self.username = u''
        #2to3 self.host = u''
        #2to3 self.port = u''
        #2to3 self.database = u''
        #2to3 self.password = u''
        #2to3 self.compressed = u''
        self.name = ''
        self.username = ''
        self.host = ''
        self.port = ''
...
class ServicesPortfolio():
    ...
    def getServices(self):
        ...
        try:
            ...
            s.endGroup()
            for name in names:
                msg = "ServicesPortfolio: %s" % (name)
                QgsMessageLog.logMessage(msg, 'CSI Atlante')    
                c = ServiceDTO()
                c.name = name
                key = "/CSIAtlante/services/" + c.name
                #2to3 c.url = s.value(key + "/url", u'', type=unicode)
                #2to3 c.usr = s.value(key + "/usr", u'', type=unicode)
                #2to3 c.pwd = s.value(key + "/pwd", u'', type=unicode)
                c.url = s.value(key + "/url", '', type=str)
                c.usr = s.value(key + "/usr", '', type=str)
                c.pwd = s.value(key + "/pwd", '', type=str)
